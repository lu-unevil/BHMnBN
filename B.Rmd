...
cp <- function(keys, ..., bib=bibf){
  keys <- gsub("^@", "", keys)
  RefManageR::Citep(bib, keys, ...)
}
ct <- function(keys, ..., bib=bibf){
  keys <- gsub("^@", "", keys)
  RefManageR::Citet(bib, keys, ...)
}

RefManageR::BibOptions(check.entries = FALSE, bib.style = "authoryear", 
                       style = "markdown", max.names=2, dashed = TRUE)
bibf <- 'BHMnBN.bib' %>% 
...
`r ct("@hansson2009CasinoJungle", .opts=list(hyperlink=FALSE))`
...
.bg-washed-green.b--dark-green.ba.bw1.br3.shadow-5.ph4.mt0[
...
.tr[
— `r ct("@fischhoff2014CommunicatingScientificUncertainty")`]
]
...
`r ct("@efsa2018PrinciplesMethodsEFSA")` The principles and methods behind EFSA's guidance on uncertainty analysis in scientific assessment.

`r ct("@sapea2019ScientificAdviceEuropean")` Scientific Advice to European Policy in a Complex World

`r ct("@instituteofmedicine2013EnvironmentalDecisionsFace")` Environmental decisions in the face of uncertainty
...
.footnote[`r ct("@vanderbles2019CommunicatingUncertaintyFacts")`]
...
.bg-washed-green.b--dark-green.ba.bw1.br3.shadow-5.ph4.mt0[These can, but don’t have to be used together. ]
]
...
.footnote[`r ct(c("@gelman2013BayesianDataAnalysis", "@kadane1999RethinkingFoundationsStatistics"))`] 
]
...
.footnote[`r ct("@borsuk2004BayesianNetworkEutrophication")`]
]
...
- Aleatory BN to build integrated assessment models to assess probability of adverse events or impacts `r cp("@borsuk2004BayesianNetworkEutrophication")`

- Epistemic BN to support the parametrisation e.g. based on multiple studies (evidence synthesis) to do probabilistic reasoning of the state of a system (effective, toxic, present, invasive etc) `r cp("@hu2020HowConductBayesian")`
